...
export const useAssetLoad = ({
    ...
    const autoLoadAssets = useCallback(async (
        ...
        try {
            ...
            if (matchingVideos.length > 0) {
                ...
                const videoUrl = await PersistenceService.getVideoPath(archiveZone, objId, String(maxIndex));
                if (videoUrl) {
            ...
            if (matchingImages.length > 0) {
                ...
                const imageUrl = await PersistenceService.loadImage(archiveZone, objId, baseName);
                if (imageUrl) {
    ...
    const handleRandomSwitch = useCallback(async (
        ...
        try {
            ...
            // 判定条件必须是「是否存在与当前显示不同的资产」，而不是「文件数 <= 1」。
            // 后者会在档案里只有唯一一份资产、而界面当前什么都没显示时（例如自动挂载失败）
            // 直接拒绝挂载，表现为「本地明明有图却切换不出来」。
            if (matchingFiles.length === 0) {
                addLog(`档案中没有可用的${isImage ? '视觉' : '视频'}资产。`, 'warning');
                return null;
            }
            ...
            for (let attempts = 0; attempts < 10; attempts++) {
                ...
                const loadedUrl = isImage
                    ? await PersistenceService.loadImage(archiveZone, objId, baseName)
                    : await PersistenceService.getVideoPath(archiveZone, objId, baseName);
            ...
            // 全部尝试都命中了当前显示的那一份 → 档案中只有唯一变体
            addLog(`档案中只有唯一一份${isImage ? '视觉' : '视频'}资产，无可切换的变体。`, 'warning');
            return null;
        } catch (error) {
