...
export const generateVisual = async (
    ...
    try {
        ...
        // 按返回值形态分派，而非按 providerId 硬编码：
        // - data: URL → 已是 base64，直接入库；
        // - http(s) URL → 下载并转 base64（覆盖 pollinations/deapi/qwenstudio 等返回 URL 的端点）；
        // - 其他 → 视作 base64 字符串。
        if (imageResult.startsWith('data:')) {
            base64Image = imageResult;
        } else if (/^https?:\/\//.test(imageResult)) {
            const response = await fetch(imageResult);
            if (!response.ok) {
                throw new AIServiceError(`远端图像流抓取失败，状态码: ${response.status}`, providerId, modelOnly);
            }
        ...
        PersistenceService.saveImage(context.zoneId, context.nodeId, filename, base64Image)
...
export const generateSpeech = async (
    ...
    const speechConfig = settings.speechModel;
    if (!speechConfig) {
        BaseProvider.logWarn('Media Pipeline', '未配置语音模型（当前服务商不提供 TTS），跳过语音合成。');
        return null;
    }
    const { provider: providerId, model: modelOnly } = speechConfig;
    ...
    try {
        ...
        PersistenceService.saveAudio(context.zoneId, context.nodeId, filename, audioData)
...
export const generateVideo = async (
    ...
    try {
        ...
        if (videoUrl) {
            ...
            PersistenceService.saveVideo(context.zoneId, context.nodeId, filename, videoUrl)
