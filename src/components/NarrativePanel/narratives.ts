 ...
 *
 * 设计原则：
 * - MODE：叙事模式，决定全局叙事状态是长线链式追踪，还是单区域单元剧闭环。
 * - PACING：叙事节奏，控制张力积累、威胁呈现方式与节点展开速度。
 * - THEME：子叙事主题，负责感官美学、空间氛围与视觉风格。
 * - MOTIF：叙事主旨，负责哲学 / 情感内核，规定事件的道德重量与象征意义。
 * - AXIS：叙事主轴，负责玩法 / 行动导向，规定节点压力结构与玩家行为驱动。
 *
 * 注意：
 * - THEME 不得侵入 MOTIF 的哲学领域。
 * - MOTIF 不得描述具体视觉美学或空间感官。
 * - AXIS 不得描述哲学内核，应聚焦玩法压力与行动逻辑。
 */
...
export const PRESET_THEMES: NarrativeThemeSub[] = [
    ...
    },
    {
        ...
        belong: 'cyber_occult',
        prompt: `[THEME: SIGNAL_ANOMALY] SENSORY: Static that resolves into voices, screen phosphor glow in darkness, carrier-wave tones at the edge of hearing. VISUAL: Banks of outdated radio and television equipment, oscilloscope displays showing waveforms that do not correspond to any input. TONE: Communications horror. Something is using dead channels. Information theory as occult practice. FORBIDDEN: Clear, legible messages. All signals must require interpretation, and interpretation must carry risk.`,
    },

    // ── 认知危害 (Cognitive Hazard) ───────────────────────────────────
    {
        id: 'Memory Palace Collapse',
        sub: '你建造它是为了记住。现在它开始遗忘你。',
        mainColor: 'text-violet-300',
        subColor: 'border-violet-800',
        belong: 'cognitive_hazard',
...
export const MOTIF_PRESETS: NarrativeMotif[] = [
    ...
    {
        id: '绝望循环',
        sub: '这不是第一次，也不会是最后一次。你的挣扎被完美地录制下来，供下一个人观看。',
        mainColor: 'text-stone-300',
        subColor: 'border-stone-800',
    ...
    {
        id: '信任的代价',
        sub: '你依赖的人，也是你最脆弱的弱点。这不是阴谋，这只是规律。',
        mainColor: 'text-stone-300',
        subColor: 'border-stone-800',
...
export const AXIS_PRESETS: NarrativeMainAxis[] = [
    ...
    // ── 调查与收集 (Investigation & Collection) ────────────────────────
    {
        id: '线索重建',
