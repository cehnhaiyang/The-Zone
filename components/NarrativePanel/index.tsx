        ...
        <BaseModal
            ...
            footerContent={
                <>
                    ...
                    {onOpenLibrary && <button onClick={() => { AudioService.playSfx('click'); onOpenLibrary(); }} className="px-6 py-2.5 border border-purple-900/50 text-purple-500 hover:text-purple-300 font-mono text-xs tracking-widest uppercase transition-all duration-300 hover:bg-purple-900/20 shadow-[0_0_15px_rgba(168,85,247,0.1)]">归档</button>}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
                {MODES_DEF.map(mode => {
                    ...
                    return (
                        <button
                            key={mode.id}
                            onClick={() => { setSelectedMode(mode.id as NarrativeMode); AudioService.playSfx('click'); }}
                ...
                <div className="flex-1 grid grid-cols-2 xl:grid-cols-4 gap-6 auto-rows-max">
                    {NARRATIVE_PACING.map(mode => {
                        ...
                        return (
                            <button
                                key={mode.id}
                                onClick={() => { setSelectedPacing(mode.id as NarrativePacing); AudioService.playSfx('hover'); }}
...
const ThemePanel: React.FC<ThemePanelProps> = ({ onConfirm, onCancel }) => {
    ...
    const handleConfirm = () => {
        AudioService.playSfx('success');
        onConfirm(themeMode === 'custom' ? customTheme : themeMode === 'preset' ? selectedThemeId : '');
    };
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 shrink-0 pb-4 border-b border-emerald-900/30">
                    ...
                    <div className="flex bg-black/50 border border-emerald-900/40 rounded-sm p-1">
                        {(['preset', 'custom', 'auto'] as const).map(mode => (
                            <button key={mode} onClick={() => { setThemeMode(mode); AudioService.playSfx('click'); }} className={`text-xs px-5 py-2 font-mono tracking-widest transition-all rounded-sm ${themeMode === mode ? 'bg-emerald-900/60 text-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'text-emerald-700/60 hover:text-emerald-500 hover:bg-emerald-900/20'}`}>{mode.toUpperCase()}</button>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
                            {PRESET_THEMES.map(theme => {
                                ...
                                return (
                                    <button
                                        key={theme.id}
                                        onClick={() => { setSelectedThemeId(theme.id); AudioService.playSfx('click'); }}
                ...
                <div className="space-y-6">
                    <div className="flex flex-wrap items-center justify-between gap-4 pb-3 border-b border-purple-900/30">
                        ...
                        <div className="flex bg-black/50 border border-purple-900/40 rounded-sm p-1">
                            {(['preset', 'custom', 'auto'] as const).map(mode => (
                                <button key={mode} onClick={() => { setMotifMode(mode); AudioService.playSfx('click'); }} className={`text-xs px-4 py-1.5 font-mono tracking-widest transition-all rounded-sm ${motifMode === mode ? 'bg-purple-900/60 text-purple-300 shadow-[0_0_10px_rgba(168,85,247,0.2)]' : 'text-purple-700/60 hover:text-purple-500 hover:bg-purple-900/20'}`}>{mode.toUpperCase()}</button>
                            ))}
                        </div>
                    </div>
                    {motifMode === 'preset' && (
                ...
                <div className="space-y-6 flex-1">
                    <div className="flex flex-wrap items-center justify-between gap-4 pb-3 border-b border-amber-900/30">
                        ...
                        <div className="flex bg-black/50 border border-amber-900/40 rounded-sm p-1">
                            {(['preset', 'custom', 'auto'] as const).map(mode => (
                                <button key={mode} onClick={() => { setAxisMode(mode); AudioService.playSfx('click'); }} className={`text-xs px-4 py-1.5 font-mono tracking-widest transition-all rounded-sm ${axisMode === mode ? 'bg-amber-900/60 text-amber-300 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : 'text-amber-700/60 hover:text-amber-500 hover:bg-amber-900/20'}`}>{mode.toUpperCase()}</button>
                            ))}
                        </div>
                    </div>
                    {axisMode === 'preset' && (
...
const PreviewPanel: React.FC<PreviewPanelProps> = ({ player, pendingConfig, episodicTension, episodicNodeCount, narrativePreview, onConfirm, onCancel }) => {
    ...
    const displayPhase = analysis?.params?.progress?.marco || 'SETUP';
    ...
    return (
        <BaseModal
            ...
            footerContent={
                <>
                    {onCancel && <button onClick={() => { AudioService.playSfx('click'); onCancel(); }} className="px-4 py-2 border border-slate-700/60 text-slate-400 font-bold text-xs tracking-wider uppercase hover:bg-slate-800/30 transition-colors">重置</button>}
...
const NarrativePanel: React.FC<NarrativePanelProps> = ({ player, narrativeLibrary, loadNarrativeLibrary, deleteNarrativeArc, deleteEpisodicZone, narrativePreview, narrativeFlowStep, narrativeFlowPendingConfig, narrativeFlowEpisodicTension, narrativeFlowEpisodicNodeCount, narrativeFlowHasSuspendedArc, setNarrativeFlowStep, setNarrativeFlowEpisodicTension, setNarrativeFlowEpisodicNodeCount, handleNarrativeFlowOpenLibrary, handleNarrativeFlowLoadFromLibrary, handleNarrativeFlowModeConfirm, handleNarrativeFlowPacingConfirm, handleNarrativeFlowThemeConfirm, handleNarrativeFlowConfigConfirm, handleNarrativeFlowDetailsConfirm, handleNarrativeFlowPreviewConfirm, handleNarrativeFlowBack, handleNarrativeFlowCancel }) => {
    if (narrativeFlowHasSuspendedArc) return null;
