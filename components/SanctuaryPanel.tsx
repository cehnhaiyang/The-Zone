...
import { PlayerState, Item, ItemInstance, Sanctuary, Node, Entity, NpcTemplate, NpcDynamicState } from '../meta';
...
const RARITY_COLORS: Record<ItemInstance['rarity'], { border: string; text: string; bg: string; shadow: string }> = {
    common: { border: 'border-gray-600/50', text: 'text-gray-400', bg: 'bg-gray-900/40', shadow: 'shadow-none' },
    rare: { border: 'border-blue-500/50', text: 'text-blue-400', bg: 'bg-blue-900/20', shadow: 'shadow-[0_0_10px_rgba(59,130,246,0.15)]' },
    epic: { border: 'border-purple-500/60', text: 'text-purple-400', bg: 'bg-purple-900/20', shadow: 'shadow-[0_0_15px_rgba(168,85,247,0.25)]' },
    cursed: { border: 'border-red-600/60', text: 'text-red-500', bg: 'bg-red-950/30', shadow: 'shadow-[0_0_20px_rgba(239,68,68,0.3)] animate-pulse' }
};
...
const ItemRow = memo(({ item, actionLabel, onClick, isStorage }: { item: ItemInstance; actionLabel: string; onClick: () => void; isStorage?: boolean }) => {
    ...
    return (
        <div className={`flex items-center justify-between p-2.5 mb-2 transition-all tech-clip group border-l-2 hover:pl-4 cursor-pointer relative overflow-hidden bg-black/40 hover:bg-[#0a0a0c] border border-transparent hover:${rarityTheme.border} ${rarityTheme.shadow}`} onClick={() => { AudioService.playSfx('click'); onClick(); }}>
            <div className={`absolute top-0 left-0 w-[2px] h-full ${rarityTheme.bg.replace('/20', '/80').replace('/30', '/80').replace('/40', '/80')} group-hover:w-full group-hover:opacity-10 transition-all duration-300`}></div>

            <div className="flex items-center gap-3 min-w-0 flex-1 relative z-10">
                <div className={`w-7 h-7 flex items-center justify-center bg-black/80 border border-current/30 p-1.5 ${rarityTheme.text} shadow-[inset_0_0_10px_rgba(0,0,0,0.5)] tech-clip`}>
                    {icon}
                </div>
                <div className="flex flex-col min-w-0">
                    <span className={`text-[11px] font-bold font-mono truncate group-hover:text-white transition-colors ${rarityTheme.text}`}>{item.name}</span>
                    <span className="text-[8px] text-gray-500 uppercase tracking-wider mt-0.5">{item.type}</span>
                </div>
            </div>

            <button className={`relative z-10 ml-3 px-2.5 py-1.5 text-[8px] font-mono border border-current/40 ${rarityTheme.text} hover:bg-current hover:text-black transition-all uppercase tracking-widest font-bold`}>
                {actionLabel}
            </button>
        </div>
    );
                ...
                <div className="space-y-5">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        {QUICK_TIME_BUTTONS.map(hours => (
                            <button
                                key={hours} onClick={() => { AudioService.playSfx('click'); setSelectedHours(hours); }}
                ...
                <div className="bg-[#050508] border border-blue-900/40 p-4.5 relative tech-clip-rev">
                    ...
                    <div className="mt-3.5 pt-3.5 border-t border-emerald-900/40 relative z-10">
                        <div className="grid grid-cols-2 gap-x-5 gap-y-2.5 text-[9px] font-mono">
                            <div className="flex justify-between"><span className="text-gray-500">HP_REGEN:</span><span className="text-emerald-400 font-bold">+{Math.floor(player.static.initialState.vital.maxHp * (previewConfig.hpRecovery || 0))}</span></div>
                            <div className="flex justify-between"><span className="text-gray-500">SAN_REGEN:</span><span className="text-blue-300 font-bold">+{Math.floor(player.static.initialState.vital.maxSanity * (previewConfig.sanityRecovery || 0))}</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-6xl mx-auto w-full relative z-10">
            ...
            <div className="flex flex-col gap-6 lg:col-span-7">
                <div className="bg-[#08080a]/95 border border-amber-900/50 p-6 relative group tech-clip shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
                    ...
                    <div className="space-y-4">
                        <CyberStatBar label="Integrity" value={player.dynamic.hp} max={player.static.initialState.vital.maxHp} color="text-emerald-400" subLabel="PHYSICAL_H" />
                        <CyberStatBar label="Sanity" value={player.dynamic.sanity} max={player.static.initialState.vital.maxSanity} color="text-blue-400" subLabel="NEURAL_STB" />
                        <CyberStatBar label="Stamina" value={player.dynamic.stamina} max={player.static.initialState.vital.maxStamina} color="text-amber-500" subLabel="ENDURANCE" />
                        <CyberStatBar label="Vigor" value={player.dynamic.vigor} max={player.static.initialState.vital.maxVigor} color="text-purple-400" subLabel="FOCUS_LVL" />
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <ResourceCard label="Food" value={sanctuary?.initial.food || 0} iconType="food" unit="Unt" />
                    <ResourceCard label="Water" value={sanctuary?.initial.water || 0} iconType="water" unit="Ltr" />
                    <ResourceCard label="Med" value={sanctuary?.initial.medicine || 0} iconType="med" unit="Dos" />
                    <ResourceCard label="Power" value={sanctuary?.initial.electricity || 0} iconType="power" unit="KW/h" />
                    <ResourceCard label="Scrap" value={sanctuary?.initial.scraps || 0} iconType="scrap" unit="Pcs" />
                    <ResourceCard label="Pop" value={sanctuary?.initial.population || 0} iconType="pop" unit="Lvng" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="bg-[#050508]/90 border border-amber-900/50 p-5 flex flex-col items-center justify-center relative overflow-hidden group tech-clip-rev shadow-lg">
                        ...
                        <div className={`text-5xl font-mono font-black tracking-tighter ${(sanctuary?.initial.erosion || 0) > 50 ? 'text-purple-400 drop-shadow-[0_0_15px_rgba(147,51,234,0.6)]' : 'text-blue-500/80 drop-shadow-[0_0_10px_rgba(59,130,246,0.2)]'}`}>{(sanctuary?.initial.erosion || 0).toFixed(1)}%</div>
                        <div className={`text-[8px] font-mono mt-3 px-2.5 py-1 border ${(sanctuary?.initial.erosion || 0) > 50 ? 'text-purple-400 bg-purple-950/50 border-purple-900/50' : 'text-blue-500 bg-blue-950/30 border-blue-900/40'}`}>{(sanctuary?.initial.erosion || 0) > 50 ? 'CRIT: CONTAMINATION' : 'MIN_INTERFERENCE'}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col h-full lg:col-span-5">
                <div className="bg-[#08080a]/95 border border-amber-900/50 p-6 flex-1 flex flex-col relative tech-clip-rev shadow-[0_10px_30px_rgba(0,0,0,0.8)]">
                    ...
                    <div className="mt-6 pt-6 border-t border-amber-900/30 relative z-10">
                        ...
                        <button
                            onClick={() => { AudioService.playSfx('click'); onUseMedicine?.(1); }}
                            disabled={(sanctuary?.initial.medicine || 0) <= 0}
                            className={`w-full py-3 flex items-center justify-center gap-2 border font-mono text-[10px] tracking-[0.2em] uppercase transition-all tech-clip font-bold ${(sanctuary?.initial.medicine || 0) > 0 ? 'bg-emerald-950/30 border-emerald-700 text-emerald-400 hover:bg-emerald-900/60 hover:text-emerald-200 shadow-[inset_0_0_15px_rgba(16,185,129,0.15)]' : 'bg-black/60 border-gray-800 text-gray-600 cursor-not-allowed'}`}
                        >
...
const StorageSection: React.FC<Pick<SanctuaryPanelProps, 'player' | 'onTransferItem' | 'onOrganizeStorage' | 'maxStorage'>> = ({
    ...
    const filters = ['all', 'weapon', 'armor', 'accessory', 'consumable', 'document', 'audio', 'material'];
            ...
            <div className="flex items-end justify-between mb-8 border-b border-amber-900/50 pb-5 relative z-10">
                <div>
                    ...
                    <div className="flex gap-2 mt-4 flex-wrap">
                        {filters.map(f => (
                            <button
                                key={f} onClick={() => { AudioService.playSfx('click'); setFilter(f); }}
                ...
                <div className="flex flex-col items-end gap-3.5">
                    ...
                    {onOrganizeStorage && (
                        <button onClick={() => { AudioService.playSfx('click'); onOrganizeStorage(); }} className="text-[9px] font-mono text-amber-600 hover:text-amber-300 border-b border-dashed border-amber-900/60 hover:border-amber-400 transition-colors uppercase tracking-widest font-bold">
                            EXEC_AUTO_SORT
                        </button>
                    )}
                </div>
...
const MapSection: React.FC<Pick<SanctuaryPanelProps, 'sanctuary'>> = ({ sanctuary }) => {
    const [selectedNodeId, setSelectedNodeId] = useState<string>(sanctuary?.initial?.entrance || '');

    const layout = useMemo(() => {
        if (!sanctuary?.nodes) return [];
        const nodes: LayoutNode[] = [];
        const visited = new Set<string>();
        const queue: { id: string, depth: number }[] = [{ id: sanctuary.initial?.entrance || '', depth: 0 }];
                            ...
                            <div key={depth} className="flex gap-16 relative">
                                ...
                                {levels[depth].map(node => (
                                    <div key={node.id} className="flex flex-col items-center group relative z-10">
                                        ...
                                        <button
                                            onClick={() => {
                                                if (node.isVisited) { setSelectedNodeId(node.id); AudioService.playSfx('click'); }
...
const SanctuaryPanel: React.FC<SanctuaryPanelProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'map' | 'storage' | 'companion'>('overview');
                    ...
                    <div className="flex items-center h-full absolute left-1/2 -translate-x-1/2">
                        {[
                            ...
                            return (
                                <button
                                    key={tab.id}
                                    onClick={() => { AudioService.playSfx('click'); setActiveTab(tab.id as any); }}
