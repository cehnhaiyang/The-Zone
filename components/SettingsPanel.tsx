...
import { Settings, ModelCategory } from '../meta';
import { MODEL_REGISTRY, MODEL_PROVIDER } from '../constants/config';
import { PersistenceService, KeyService, AudioService } from '../services';
import { PlatformHealthSnapshot, KeyHealthSnapshot } from '../services/KeyService';
...
interface SettingsPanelProps {
    ...
    onListSaves?: () => Promise<Array<{ name: string; size: number; modified: Date }>>;
    onDeleteSave?: (fileName: string) => Promise<boolean>;
}

const COLOR_STYLES: Record<string, any> = {
    emerald: { text: "text-emerald-400", border: "border-emerald-500/50", borderActive: "border-emerald-500", bg: "bg-emerald-950/40", accent: "accent-emerald-500", glow: "shadow-[0_0_10px_rgba(16,185,129,0.3)]" },
    teal: { text: "text-teal-400", border: "border-teal-500/50", borderActive: "border-teal-500", bg: "bg-teal-950/40", accent: "accent-teal-500", glow: "shadow-[0_0_10px_rgba(45,212,191,0.3)]" },
    sky: { text: "text-sky-400", border: "border-sky-500/50", borderActive: "border-sky-500", bg: "bg-sky-950/40", accent: "accent-sky-500", glow: "shadow-[0_0_10px_rgba(56,189,248,0.3)]" },
    blue: { text: "text-blue-400", border: "border-blue-500/50", borderActive: "border-blue-500", bg: "bg-blue-950/40", accent: "accent-blue-500", glow: "shadow-[0_0_10px_rgba(59,130,246,0.3)]" },
    purple: { text: "text-purple-400", border: "border-purple-500/50", borderActive: "border-purple-500", bg: "bg-purple-950/40", accent: "accent-purple-500", glow: "shadow-[0_0_10px_rgba(168,85,247,0.3)]" },
    amber: { text: "text-amber-400", border: "border-amber-500/50", borderActive: "border-amber-500", bg: "bg-amber-950/40", accent: "accent-amber-500", glow: "shadow-[0_0_10px_rgba(245,158,11,0.3)]" },
    orange: { text: "text-orange-400", border: "border-orange-500/50", borderActive: "border-orange-500", bg: "bg-orange-950/40", accent: "accent-orange-500", glow: "shadow-[0_0_10px_rgba(249,115,22,0.3)]" },
    red: { text: "text-red-400", border: "border-red-500/50", borderActive: "border-red-500", bg: "bg-red-950/40", accent: "accent-red-500", glow: "shadow-[0_0_10px_rgba(239,68,68,0.3)]" },
    cyan: { text: "text-cyan-400", border: "border-cyan-500/50", borderActive: "border-cyan-500", bg: "bg-cyan-950/40", accent: "accent-cyan-500", glow: "shadow-[0_0_10px_rgba(6,182,212,0.3)]" },
};
...
const TabButton = ({ active, label, onClick }: { active: boolean, label: string, onClick: () => void }) => (
    <button
        onClick={() => { onClick(); AudioService.playSfx('click'); }}
        className={`relative flex-1 py-4 text-[11px] uppercase tracking-[0.2em] font-bold transition-all duration-300 overflow-hidden
            ${active ? 'text-emerald-400 bg-emerald-950/20 text-shadow-sm'
...
const ModelSelector = ({ label, category, current, colorClass = "emerald", onChange }: {
    label: string;
    ...
    const style = COLOR_STYLES[colorClass] || COLOR_STYLES.emerald;
    const modelOptions = MODEL_REGISTRY[category] as Array<{ provider: string, model: string, label?: string }>;
                            ...
                            <div className="bg-black/40 border border-zinc-800/60 p-0.5 rounded-sm">
                                {options.map((opt) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="tracking-wider z-10">{displayLabel}</span>
                                            {isActive && <span className="text-[9px] opacity-90 animate-pulse font-bold tracking-widest z-10">● ACTV</span>}
                                            {!isActive && <span className="text-[9px] opacity-0 group-hover/btn:opacity-50 transition-opacity z-10 tracking-widest">SELECT</span>}
                                        </button>
                                    );
                                })}
                            </div>
...
const RangeSlider = ({ label, value, min, max, step, colorClass, onChange, displayValue }: any) => {
    const style = COLOR_STYLES[colorClass] || COLOR_STYLES.emerald;

    return (
        <div className="group mb-5 last:mb-0 bg-black/20 p-3 border border-zinc-800/40 rounded-sm hover:border-zinc-700/60 transition-colors">
            <div className="flex justify-between items-center mb-2">
                <label className={`text-[10px] text-zinc-400 font-bold uppercase tracking-wider transition-colors ${style.hover}`}>
                    {label}
                </label>
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'uplink' | 'cortex' | 'balance' | 'visual' | 'audio' | 'saves'>('uplink');

    const [audioParams, setAudioParams] = useState(AudioService.getMixParams());
    const [savesList, setSavesList] = useState<Array<{ name: string; size: number; modified: Date }>>([]);
    const [saveName, setSaveName] = useState('');
    const [healthSnapshot, setHealthSnapshot] = useState<Record<string, PlatformHealthSnapshot>>({});

    // 本地 Key 字符串状态同步
    const [keyStrings, setKeyStrings] = useState<Record<string, string>>(() => {
        // 遍历 MODEL_PROVIDER 泛化初始化，新增服务商无需手动追加
        const init: Record<string, string> = {};
        for (const p of MODEL_PROVIDER) {
            if (p.keyField) {
                init[p.keyField] = ((settings as any)[p.keyField] ?? []).join('\n');
            }
        }
        return init;
    });
    ...
    useEffect(() => {
        if (activeTab === 'uplink') {
            const updateHealth = () => setHealthSnapshot(KeyService.getHealthSnapshot());
            updateHealth();
            const timer = setInterval(updateHealth, 3000);
    ...
    const dispatchUpdate = useCallback((newSettings: Settings) => {
        ...
        if (Math.random() > 0.7) AudioService.playSfx('click');
    }, [onUpdate]);

    const handleChange = (key: keyof Settings, value: any) => {
        dispatchUpdate({ ...settings, [key]: value });
        if (typeof value === 'string' && (key.includes('Key') || key.includes('Id')) && !(key in keyStrings)) {
            localStorage.setItem(key, value);
        }
    };

    const handleKeyStrChange = (key: keyof typeof keyStrings, value: string) => {
        setKeyStrings(prev => ({ ...prev, [key]: value }));
        const arr = value.split('\n').map(s => s.trim()).filter(Boolean);
        dispatchUpdate({ ...settings, [key]: arr });
    };

    const handleNestedChange = (categoryPath: string, key: string, value: number) => {
        // 使用深拷贝避免直接修改 state，导致 React 无法侦测更新
        const newSettings = JSON.parse(JSON.stringify(settings));
        const parts = categoryPath.split('.');
        let target: any = newSettings.gameConfig;
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {MODEL_PROVIDER.filter(p => p.keyField).map(({ id, keyField }) => {
                    ...
                    const colorMap: Record<string, string> = {
                        google: 'blue', groq: 'orange', zhipu: 'purple',
                        pollinations: 'emerald', chat2api: 'sky'
                    };
                    const color = colorMap[id] || 'emerald';
                    const style = COLOR_STYLES[color];
                    return (
                        <div key={id} className="relative group">
                            <label className={`block text-[10px] text-zinc-500 mb-2 font-bold group-hover:${style.text} tracking-widest transition-colors flex items-center gap-2`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${style.bg} group-hover:animate-pulse`} />
                                {name} KEYS
                            </label>
                            <textarea
                                rows={3}
                                value={keyStrings[keyField as keyof typeof keyStrings] ?? ''}
                                onChange={(e) => handleKeyStrChange(keyField as keyof typeof keyStrings, e.target.value)}
                                className={`w-full bg-black/40 border border-zinc-800/80 rounded-sm py-3 px-4 text-[11px] font-mono tracking-widest focus:outline-none focus:bg-zinc-900/60 focus:border-b-2 ${style.text} placeholder-zinc-800 resize-y min-h-[4rem] transition-all`}
                                style={{ borderBottomColor: (keyStrings[keyField as keyof typeof keyStrings] ?? '') ? `var(--tw-${color}-500)` : undefined }}
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/50 p-5 rounded-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {MODEL_PROVIDER.filter(p => p.proxyField).map(({ id, proxyField }) => {
                        const name = id;
                        const isChecked = proxyField && (settings as any)[proxyField] !== false;
                        return (
                            <div key={id} className="flex items-center gap-3 group cursor-pointer" onClick={() => proxyField && handleChange(proxyField as any, !isChecked)}>
            ...
            <div className="pt-4">
                <div className="text-[10px] text-zinc-500 uppercase tracking-widest mb-4 flex justify-between items-end border-b border-zinc-800/60 pb-2">
                    ...
                    <button onClick={() => KeyService.registerFromSettings(settings)} className="text-emerald-500 hover:text-emerald-300 hover:text-shadow-sm transition-colors tracking-[0.2em]">
                        [ 重新握手并校验 ]
                    </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(healthSnapshot).map(([platform, snapshot]) => {
                        const platformName = platform;
                        return (
                            <div key={platform} className="bg-black/30 border border-zinc-800/60 p-4 rounded-sm relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-center mb-4">
                                    <span className="text-[11px] font-bold text-zinc-300 uppercase tracking-widest group-hover:text-white transition-colors">{platformName}</span>
                                ...
                                <div className="space-y-2.5">
                                    {snapshot.keys.map((key: KeyHealthSnapshot, idx: number) => (
                                        <div key={idx} className="flex items-center gap-3 text-[10px] font-mono bg-zinc-950/50 p-1.5 rounded-sm border border-zinc-900/50">
                                            ...
                                            <span className="text-zinc-400 min-w-[60px] text-right">
                                                <span className="text-emerald-500/80">{key.successCount}</span> / <span className="text-red-500/80">{key.failureCount}</span>
                                            </span>
                                    ...
                                    {snapshot.keys.length === 0 && (
                                        <div className="text-[10px] text-zinc-600 font-mono py-2 opacity-50">:: 无可用神经干线 ::</div>
                                    )}
                                </div>
                            </div>
                        )
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                <div className="space-y-2">
                    ...
                    <ModelSelector label="世界构建" category="world" current={settings.zoneModel} colorClass="emerald" onChange={(v) => handleChange('zoneModel', v)} />
                    <ModelSelector label="实体心智" category="npcDialogue" current={settings.npcDialogueModel} colorClass="teal" onChange={(v) => handleChange('npcDialogueModel', v)} />
                    <ModelSelector label="即时反射" category="npcReaction" current={settings.npcReactionModel} colorClass="teal" onChange={(v) => handleChange('npcReactionModel', v)} />
                    <ModelSelector label="动态叙事" category="dyNarrative" current={settings.dyNarrativeModel} colorClass="teal" onChange={(v) => handleChange('dyNarrativeModel', v)} />
                </div>
                <div className="space-y-2">
                    ...
                    <ModelSelector label="视觉皮层" category="image" current={settings.imageModel} colorClass="blue" onChange={(v) => handleChange('imageModel', v)} />
                    <ModelSelector label="幻觉重影" category="video" current={settings.videoModel} colorClass="purple" onChange={(v) => handleChange('videoModel', v)} />
                    <ModelSelector label="听觉回响" category="speech" current={settings.speechModel} colorClass="amber" onChange={(v) => handleChange('speechModel', v)} />
                    <ModelSelector label="记忆嵌入" category="embedding" current={settings.embeddingModel} colorClass="sky" onChange={(v) => handleChange('embeddingModel', v)} />
                </div>
            </div>
        </div>
    );
    ...
    const renderBalanceTab = () => {
        const createGroup = (title: string, color: string, categoryPath: string, configs: any[]) => {
            const style = COLOR_STYLES[color] || COLOR_STYLES.emerald;
            const parts = categoryPath.split('.');
            let configData: any = settings.gameConfig;
            for (const part of parts) {
                if (!configData || typeof configData !== 'object') return null; // Safe fallback
                configData = configData[part];
            }
                    ...
                    <div className="space-y-1">
                        {configs.map(c => (
                            <RangeSlider key={c.k} label={c.label} min={c.min} max={c.max} step={c.step} colorClass={color}
                                ...
                                onChange={(val: number) => handleNestedChange(categoryPath, c.k, val)}
                            />
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-5 bg-black/40 border border-zinc-800/60 rounded-sm relative group">
                        ...
                        <div className="space-y-1">
                            <RangeSlider label="移动体力消耗" min={0} max={10} step={1} colorClass="red" value={settings.gameConfig.movementCosts.baseStamina} onChange={(v: number) => handleNestedChange('movementCosts', 'baseStamina', v)} />
                            <RangeSlider label="移动理智消耗" min={0} max={10} step={1} colorClass="red" value={settings.gameConfig.movementCosts.baseSanity} onChange={(v: number) => handleNestedChange('movementCosts', 'baseSanity', v)} />
                            <RangeSlider label="提示消耗理智" min={0} max={10} step={1} colorClass="red" value={settings.gameConfig.hintCosts.sanityCost} onChange={(v: number) => handleNestedChange('hintCosts', 'sanityCost', v)} />
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    {createGroup("行为交互收益 (增益/损耗)", "amber", "social.benefits", [
                        ...
                        { label: "亲密理智增益", k: 'intimacySanityGain', min: 0, max: 100, step: 1 },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="space-y-6">
                <div className="bg-black/40 p-6 border border-zinc-800/60 rounded-sm">
                    ...
                    <RangeSlider label="HUD 投影亮度" value={settings.screenBrightness} min={50} max={150} step={1} colorClass="cyan" displayValue={`${settings.screenBrightness}%`} onChange={(v: number) => handleChange('screenBrightness', v)} />
                </div>
            </div>
    ...
    const renderAudioTab = () => {
        const updateMix = (key: keyof typeof audioParams, val: number) => {
            const newParams = { ...audioParams, [key]: val };
            setAudioParams(newParams);
            AudioService.setMixParams({ [key]: val });
        };
        ...
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="space-y-1">
                        {[
                            ...
                            <RangeSlider key={key} label={label} min={0} max={100} step={1} colorClass={color}
                                value={(audioParams as any)[key] * 100}
                                displayValue={`${Math.round((audioParams as any)[key] * 100)}%`}
                                onChange={(val: number) => updateMix(key as any, val / 100)}
                            />
                        ))}
                    </div>
                </div>
                <div className="space-y-6">
                    <div className="bg-black/30 p-6 border border-zinc-800/50 rounded-sm">
                        <RangeSlider label="空间混响深度 (REVERB)" min={0} max={100} step={1} colorClass="cyan" value={audioParams.reverbMix * 100} displayValue={`${Math.round(audioParams.reverbMix * 100)}%`} onChange={(v: number) => updateMix('reverbMix', v / 100)} />
                    </div>
                    ...
                    <div className="bg-black/30 p-6 border border-zinc-800/50 rounded-sm">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {['click', 'success', 'scare', 'heartbeat', 'glitch', 'combat_hit'].map((sfx) => (
                                <button key={sfx} onClick={() => AudioService.playSfx(sfx as any)} className="py-2.5 px-2 text-[9px] font-mono uppercase tracking-widest border border-zinc-800 bg-black text-zinc-400 hover:text-emerald-400 hover:border-emerald-700/50 hover:bg-emerald-950/30 hover:shadow-[0_0_10px_rgba(16,185,129,0.1)] transition-all rounded-sm truncate">
                                    {sfx}
                                </button>
                            ))}
                        </div>
                    ...
                    <button
                        onClick={() => { AudioService.toggleMute(); setAudioParams({ ...audioParams }); }}
                        className={`w-full py-4 border transition-all duration-300 text-[11px] font-bold tracking-[0.2em] uppercase rounded-sm flex items-center justify-center gap-3
                            ${AudioService.getMuted() ? 'border-red-700/80 bg-red-950/40 text-red-400 hover:bg-red-900/60 hover:shadow-[0_0_15px_rgba(239,68,68,0.2)]'
                    ...
                    >
                        {AudioService.getMuted() ? (
                            <><span className="text-lg">🔇</span> 听觉系统已阻断 - 点击重启</>
                        ) : (
                            <><span className="text-lg">🔊</span> 听觉系统运行中 - 点击阻断</>
                        )}
                    </button>
                </div>
            </div>
        ...
        <div className="h-full flex flex-col max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <button
                        onClick={async () => {
                            if (onSaveGame) {
                                ...
                                await onSaveGame(saveName || undefined);
                                setSaveName('');
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar border border-zinc-800/50 bg-black/20 p-2 rounded-sm">
                <div className="flex justify-between items-center px-2 py-3 mb-2 border-b border-zinc-800/60 sticky top-0 bg-zinc-950/90 backdrop-blur-sm z-10">
                    ...
                    <button onClick={async () => { if (onListSaves) { AudioService.playSfx('click'); setSavesList(await onListSaves()); } }} className="text-[10px] text-cyan-500/80 hover:text-cyan-300 tracking-wider flex items-center gap-1 transition-colors">
                        <span>⟳</span> 重新扫描
                    </button>
                </div>
                {savesList.length === 0 ? (
                        ...
                        {savesList.map((save) => (
                            <div key={save.name} className="flex items-center justify-between p-3.5 bg-zinc-900/30 border border-zinc-800/40 hover:border-zinc-600 hover:bg-zinc-800/50 group transition-all rounded-sm">
                                ...
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={async () => { if (onLoadGame && await onLoadGame(save.name)) onClose(); }} className="px-4 py-1.5 text-[10px] font-bold tracking-widest border border-emerald-900/50 bg-emerald-950/30 text-emerald-400 hover:bg-emerald-900/60 hover:border-emerald-500 transition-colors rounded-sm uppercase">唤醒</button>
                                    <button onClick={async () => { if (onDeleteSave && confirm(`即将擦除档案 "${save.name}"，无法恢复。确认执行?`)) { await onDeleteSave(save.name); if (onListSaves) setSavesList(await onListSaves()); } }} className="px-4 py-1.5 text-[10px] font-bold tracking-widest border border-red-900/50 bg-red-950/30 text-red-400 hover:bg-red-900/60 hover:border-red-500 transition-colors rounded-sm uppercase">擦除</button>
                                </div>
                            </div>
                        ))}
                    </div>
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/80 backdrop-blur-xl">
            ...
            <div className={`relative flex flex-col bg-zinc-950/90 border border-zinc-800 shadow-[0_0_80px_rgba(16,185,129,0.05)] overflow-hidden ${isFullscreen ? 'w-full h-full' : 'w-full max-w-6xl h-[88vh] m-4 rounded-sm'}`}>
                ...
                {/* 顶部消息提示 (Toast) */}
                {msg && (
                    <div className={`absolute top-0 inset-x-0 py-2.5 text-center text-[10px] font-mono font-bold tracking-[0.3em] z-50 border-b animate-in slide-in-from-top-2 shadow-lg ${msg.type === 'success' ? 'bg-emerald-950/90 text-emerald-400 border-emerald-800 shadow-emerald-900/20' : msg.type === 'warning' ? 'bg-amber-950/90 text-amber-400 border-amber-800 shadow-amber-900/20' : 'bg-red-950/90 text-red-400 border-red-800 shadow-red-900/20'} backdrop-blur-md uppercase`}>
                        {msg.text}
                    </div>
                )}
                ...
                <div className="shrink-0 h-16 flex items-center justify-between px-8 border-b border-zinc-800/80 bg-zinc-900/50 z-10 relative">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                            <h2 className="text-[16px] font-mono text-zinc-100 tracking-[0.4em] uppercase font-bold text-shadow">系统控制枢纽</h2>
                            <span className="text-[8px] text-zinc-500 font-mono tracking-widest mt-0.5">SYS.CNTRL.OVERRIDE_V2.1</span>
                        </div>
                    </div>
                ...
                <div className="shrink-0 px-6 py-4 border-t border-zinc-800/80 bg-black flex justify-between items-center z-10 text-[10px] font-mono">
                    <div className="flex gap-8">
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="text-zinc-300 font-bold">{Math.round(Math.random() * 40 + 20)}%</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-zinc-600 tracking-widest">CPU_LOAD</span>
                            <span className="text-zinc-300 font-bold">{Math.round(Math.random() * 30 + 10)}%</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-zinc-600 tracking-widest">NET_STATUS</span>
                            <span className="text-emerald-500 font-bold animate-pulse shadow-emerald-500/20">SECURE_LINK</span>
                        </div>
                    </div>
